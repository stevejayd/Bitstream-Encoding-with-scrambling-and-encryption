...
//***************************** ENCODING ********************************
void EncodeUnaryRandom(std::string& InputBytes, std::string& FileName)
{
    ...
    */
    ...
    for (i = 0;i < NumberOfUnaryBits;i++)
    {
        ...
        TestBit = EncodedBits.test(i);
        CharTemp = CharTemp + TestBit * j;
        j >>= 1;
        if (ii == 8)
        {
            ii = 0;
            j = 0x80;
            BytesToFile[NumberBytes] = CharTemp;
            CharTemp = 0;
            BitCount = i;//last bit
            NumberBytes++;
        }
    };
    // finish up left over bits.. xxxx0000; 
    if (ii > 0) // 
    {
        BytesToFile[NumberBytes] = CharTemp;
        NumberBytes++;
    }
    //************** BytesToFile *** conatins just message data
    ByteMask = 0x8000;
    ii = 0;
    for (i = 0;i < 16;i++)
    {
        ii = ii + ByteMask & NumberBytes;
        ByteMask >>= 1;
        if (i == 7)
        {
            HighByte = ii;
    ...
    myfile.open(FileName, std::ofstream::trunc | std::ofstream::binary);// erase old file then write
    if (!myfile)
    {
    ...
    else
    {
        ...
        myfile << HighByte;// unsigned int
        myfile << LowByte;// unsigned int
        // Swapped or not "0","1'
        CharTemp = SwapChunks;
        myfile << CharTemp;
        CharTemp = XorBits;
        myfile << CharTemp;
        myfile.close();// finished writing bytes


        myfile.open(FileName, std::ofstream::app | std::ofstream::binary);


        for (i = 0;i < NumberBytes;i++)
        {
            myfile << BytesToFile[i];
        };
        myfile.close();
    }
    //************************************************* Read File **************************************
